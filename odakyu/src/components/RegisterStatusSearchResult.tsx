...
import {
  ...
  RegisterItem,
} from './types/RegisterStatusResult'
    ...
    <>
      ...
      >
        ...
        >
          ...
          <TableBody>
            <TableRow>
              <TableCell>
                {props.items?.registerdMail
                  ? props.items?.registerdMail
                  : '未登録'}
              </TableCell>
              <TableCell>
                {props.items?.oneUid ? props.items?.oneUid : '-'}
              </TableCell>
              <TableCell>
                {props.items?.isPaidUser
                  ? props.items?.isPaidUser
                    ? '有償'
                    : 'フリー'
                  : '-'}
        ...
        <Text color="black">
          {
            Array.from(
              new Map(
                props.items?.registerItems.map((x: RegisterItem) => [
                  x.deliveryMail,
                  x.deliveryMail,
                ]),
              ).values(),
      ...
      >
        ...
        >
          ...
          <TableBody>
            {props.items?.registerItems?.map(
              (item: RegisterItem, index: React.Key) => {
